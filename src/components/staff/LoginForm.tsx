...
// This deployment is the admin-only portal; the route itself signs out
// any non-admin role rather than leaving that check to the client.
// Generic error messages throughout — never reveal whether a given Admin
// ID exists.
async function submitStaffLogin(credentials: { staffId: string; password: string }): Promise<LoginResult> {
  ...
  if (!response.ok || !data.role) {
    return { ok: false, message: data.error ?? "Invalid Admin ID or password." };
  }
